		...
		if ((<player> = 2) && ($p2_ready = 0))
			...
			if (($is_network_game) = 1)
				if ScreenElementExists \{id = ready_container_p2}
					RunScriptOnScreenElement \{id = ready_container_p2 pos2 = (825.0, 450.0) drop_in_ready_sign params = {player = 2}}
				endif
			else
...
script character_hub_select_change_outfit 
	if ((($is_network_game) = 1) && (<player> = 1))
		network_player_lobby_message \{type = character_hub_select action = select value1 = 1}
	endif
	if ($is_network_game)
		net_info_panel_hide
	endif
...
script character_hub_go_back 
	if ($current_num_players = 2)
		...
		if (<leaving_character_hub> = 0)
			if (<player> = 1)
				if ($is_network_game = 1)
					network_player_lobby_message \{type = ready_up action = deselect}
				endif
