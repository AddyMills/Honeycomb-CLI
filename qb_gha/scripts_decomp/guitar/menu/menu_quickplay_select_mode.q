...
script create_select_quickplay_mode 
	...
	CreateScreenElement \{type = ContainerElement parent = root_window id = mqsm_container Pos = (0.0, 0.0)}
	CreateScreenElement \{type = SpriteElement parent = mqsm_container texture = toprockers_poster Pos = (740.0, 30.0) just = [RIGHT top] dims = (620.0, 620.0) z_priority = 1.2 rot_angle = -3}
	...
	new_menu \{scrollid = scrolling_select_quickplay_mode vmenuid = vmenu_select_quickplay_mode menu_pos = (640.0, 260.0) use_backdrop = 0 exclusive_device = $primary_controller event_handlers = [{pad_back ui_flow_manager_respond_to_action params = {action = go_back}}{pad_up generic_menu_up_or_down_sound params = {up}}{pad_down generic_menu_up_or_down_sound params = {down}}] exclusive_device = $primary_controller}
	CreateScreenElement \{type = ContainerElement parent = vmenu_select_quickplay_mode dims = (100.0, 100.0) event_handlers = [{focus mqsm_focus params = {id = single_quickplay_text}}{unfocus mqsm_unfocus params = {id = single_quickplay_text}}{pad_choose ui_flow_manager_respond_to_action params = {action = select_p1_quickplay}}] exclusive_device = $primary_controller}
	CreateScreenElement \{type = ContainerElement parent = vmenu_select_quickplay_mode dims = (100.0, 100.0) event_handlers = [{focus mqsm_focus params = {id = coop_quickplay_text}}{unfocus mqsm_unfocus params = {id = coop_quickplay_text}}{pad_choose ui_flow_manager_respond_to_action params = {action = select_p2_quickplay}}] exclusive_device = $primary_controller}
	new_menu \{scrollid = scrolling_tr_select_difficulty_mode vmenuid = vmenu_tr_select_difficulty_mode menu_pos = (710.0, 200.0) use_backdrop = 0 exclusive_device = $primary_controller internal_just = [left center] event_handlers = [{pad_back mqsm_unfocus_difficulty_menu}{pad_back ui_flow_manager_respond_to_action params = {action = go_back}}{pad_up generic_menu_up_or_down_sound params = {up}}{pad_down generic_menu_up_or_down_sound params = {down}}] exclusive_device = $primary_controller}
	<i> = 0
	<bar_pos> = (850.0, 243.0)
	begin
	...
	LaunchEvent \{type = unfocus target = vmenu_tr_select_difficulty_mode}
...
script mqsm_focus_difficulty_menu 
	LaunchEvent \{type = unfocus target = vmenu_select_quickplay_mode}
	if ($g_mqsm_do_flash)
		KillSpawnedScript \{name = mqsm_focus_flash}
	endif
	if ($top_rockers_which_mode = p1_quickplay)
		...
		DoScreenElementMorph \{id = coop_quickplay_text alpha = 0.6 time = 0.15}
	else
		DoScreenElementMorph id = coop_quickplay_text rgba = (($g_menu_colors).lt_violet_bar) Scale = 1.0 time = 0.15
		DoScreenElementMorph \{id = single_quickplay_text alpha = 0.6 time = 0.15}
	endif
	change \{g_mqsm_created = 1}
	time = 0.3
	<i> = 0
	...
	LaunchEvent \{type = focus target = vmenu_tr_select_difficulty_mode}
endscript
...
script mqsm_unfocus_difficulty_menu 
	...
	LaunchEvent \{type = unfocus target = vmenu_tr_select_difficulty_mode}
	DoScreenElementMorph \{id = single_quickplay_text alpha = 1 time = 0.15}
	DoScreenElementMorph \{id = coop_quickplay_text alpha = 1 time = 0.15}
	time = 0.3
	<i> = 0
	begin
	FormatText checksumname = bar_id 'bar_%i' i = <i>
	...
	LaunchEvent \{type = focus target = vmenu_select_quickplay_mode}
endscript
