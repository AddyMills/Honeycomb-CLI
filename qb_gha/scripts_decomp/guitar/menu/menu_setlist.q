...
script setlist_footnote \{alpha = 1}
	if NOT ScreenElementExists \{id = setlist_overlay}
		...
		CreateScreenElement \{type = ContainerElement parent = setlist_overlay id = setlist_footer Pos = (640.0, 645.0) z_priority = 1000 just = [center bottom] alpha = 1}
		CreateScreenElement \{type = ContainerElement parent = setlist_footer id = footer_gradient Pos = (0.0, -17.0) z_priority = 1000 just = [center bottom] alpha = 1 rot_angle = 90}
		CreateScreenElement \{type = SpriteElement parent = footer_gradient texture = gradient_128 dims = (33.0, 600.0) Pos = (0.0, 0.0) rgba = [0 0 0 100] just = [center bottom] alpha = 1}
		CreateScreenElement \{type = SpriteElement parent = footer_gradient texture = gradient_128 dims = (33.0, 600.0) Pos = (0.0, 0.0) rgba = [20 0 10 100] just = [center top] alpha = 1 flip_h}
		CreateScreenElement {
			type = TextElement
	...
	if (<alpha> = 1)
		...
		DoScreenElementMorph \{id = setlist_overlay alpha = 1 time = 0.25}
	else
		SetScreenElementProps \{id = setlist_footer_text text = "*COVER OF LISTED BAND'S HIT"}
		DoScreenElementMorph \{id = setlist_overlay alpha = 0.85 time = 0.25}
	endif
endscript
	...
	if NOT ($current_setlist_songpreview = $target_setlist_songpreview)
		...
		if ($target_setlist_songpreview != <song> || $target_setlist_songpreview = None)
			...
			if StructureContains structure = <song_struct> name = band_playback_volume
				...
				setsoundbussparams \{Guitar_Balance = {vol = -1.5}}
			else
				setsoundbussparams \{music_setlist = {vol = 0.0}}
				setsoundbussparams \{Band_Balance = {vol = -1.5}}
				setsoundbussparams \{Guitar_Balance = {vol = -1.5}}
			endif
...
script create_sl_assets 
	...
	CreateScreenElement \{type = windowelement parent = setlist_menu id = setlist_scroll_text_window Pos = (400.0, 465.0) dims = (430.0, 3240.0)}
	...
	begin
	displaySprite \{parent = setlist_menu tex = Setlist_Shoeprint Pos = (850.0, -70.0) dims = (640.0, 768.0) alpha = 0.15 z = 3.2 flip_v rot_angle = 10 BlendMode = subtract z = 3.175}
	repeat 3
	...
	if GotParam \{tab_bonus}
		CreateScreenElement \{type = TextElement parent = setlist_menu id = sl_text_1 text = "The Vault" font = text_a10 Scale = 2 Pos = (330.0, 220.0) just = [left top] rgba = [50 30 20 255] z_priority = $setlist_text_z}
...
script setlist_show_helperbar \{text_option1 = "THE VAULT" button_option1 = "\\b7" spacing = 16}
