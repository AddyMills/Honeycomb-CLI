...
script create_calibrate_lag_menu \{from_in_game = 1}
	...
	if ($calibrate_lag_section = video)
		LaunchEvent \{type = pad_choose target = calibrate_calibrate_option}
...
script calibrate_lag_fill_options \{z = 100}
	...
	GetScreenElementDims id = <id>
	fit_text_in_rectangle id = <id> dims = ((340.0, 0.0) + <height> * (0.0, 1.0)) only_if_larger_x = 1
	CreateScreenElement \{type = ContainerElement parent = cl_vmenu id = calibrate_manual_option_audio event_handlers = [{focus menu_calibrate_focus params = {index = 2}}{unfocus menu_calibrate_unfocus params = {index = 2}}{pad_choose menu_calibrate_lag_manual_choose params = {audio}}]}
	<container_id> = <id>
	CreateScreenElement {
		<text_params>
		parent = <container_id>
	...
	CreateScreenElement \{type = ContainerElement parent = cl_vmenu id = calibrate_manual_option event_handlers = [{focus menu_calibrate_focus params = {index = 3}}{unfocus menu_calibrate_unfocus params = {index = 3}}{pad_choose menu_calibrate_lag_manual_choose}]}
	<container_id> = <id>
	CreateScreenElement {
		<text_params>
		parent = <container_id>
...
script menu_calibrate_focus 
	...
	if (<index> = 0)
		...
		DoScreenElementMorph \{id = cl_manual_adjust_up_arrow_audio rgba = $menu_focus_color}
		DoScreenElementMorph \{id = cl_manual_adjust_down_arrow_audio rgba = $menu_focus_color}
	else
		obj_getid
		retail_menu_focus id = {<ObjID> child = 0}
		SetScreenElementProps \{id = calibrate_lag_hilite Pos = $calibrate_lag_hilite_pos3 dims = $calibrate_lag_hilite_dims3}
...
script menu_calibrate_lag_create_circles 
	playsound \{Heavy_Crowd_Loop_Good_01 buss = `default` vol = 1 num_loops = -1}
...
script menu_calibrate_lag_reset_lag 
	...
	if IsWinPort
		...
		SetGlobalTags \{user_options params = {lag_calibration = 1000}}
	endif
	calibrate_lag_update_text
	change \{calibrate_lag_dirty = 1}
endscript
...
script menu_calibrate_lag_manual_choose 
	...
	RunScriptOnScreenElement \{id = calibrate_lag_hilite pulse_lag_hilite}
...
script pulse_lag_hilite 
	...
	if (<rgba_match> = 1)
		DoMorph \{id = calibrate_lag_hilite alpha = 0.6 time = 0.4 motion = ease_out}
		DoMorph \{id = calibrate_lag_hilite alpha = 1 time = 0.6 motion = ease_in}
	else
		DoMorph \{id = calibrate_lag_hilite alpha = 1 time = 0.2 motion = ease_out}
		break
	endif
	repeat
endscript
...
script menu_calibrate_lag_manual_up 
	...
	if GotParam \{audio}
		...
		if menu_calibrate_lag_adjust \{value = 1 for_audio = 1}
			do_morph = 1
		endif
	else
...
script menu_calibrate_lag_adjust \{value = 1 for_audio = 0}
	if IsWinPort
		<temp_value> = <value>
		WinPortGetAudioVideoLag
		<lag_calibration> = <value>
