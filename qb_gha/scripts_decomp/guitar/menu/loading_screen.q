...
script create_loading_screen \{mode = play_song}
	...
	getrandomvalue \{name = global_or_not a = 0 b = 1 integer}
	if (<global_or_not> = 0)
	...
	if (<mode> = play_song || <mode> = play_encore || <mode> = play_boss || <mode> = restart_song)
		...
		CreateScreenElement \{type = TextElement parent = root_window id = loading_text text = "LOADING..." font = text_a10 Scale = 2.0 just = [RIGHT center] Pos = (1000.0, 580.0) rgba = [255 255 255 255] z_priority = 5000 shadow shadow_offs = (3.0, 3.0) shadow_rgba = [0 0 0 255]}
