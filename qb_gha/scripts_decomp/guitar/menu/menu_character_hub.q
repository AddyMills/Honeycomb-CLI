...
script character_hub_highlighter \{comi = 1 chh_mp = 0 chh_player = 1}
	if ($current_num_players = 1)
		retail_menu_focus
	endif
	if (<chh_mp> = 0)
		...
		if ((<player> = 1) && ($p1_ready = 0))
			...
			if ($is_network_game = 1)
				if ScreenElementExists \{id = ready_container_p1}
					RunScriptOnScreenElement \{id = ready_container_p1 pos2 = (825.0, 450.0) drop_in_ready_sign params = {player = 1}}
				endif
			else
...
script net_info_panel_hide 
	if ScreenElementExists \{id = net_game_info_panel}
		DoScreenElementMorph \{id = net_game_info_panel alpha = 0.0 time = 0.1}
	endif
endscript
...
script net_info_panel_show 
	...
	if (<can_show> = 1)
		DoScreenElementMorph \{id = net_game_info_panel alpha = 1.0 time = 0.1}
	endif
endscript
