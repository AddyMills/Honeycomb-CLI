...
script difficulty_analyzer_show \{song_name = barracuda}
	...
	CreateScreenElement \{type = ContainerElement parent = root_window id = diff_analyzer_container Pos = (0.0, 0.0)}
	CreateScreenElement \{type = SpriteElement parent = diff_analyzer_container Pos = (0.0, 0.0) just = [left top] dims = (1280.0, 720.0) rgba = [0 0 0 255]}
...
script calc_difficulty_from_all_song_sections \{song_array = None marker_array = None fretbar_array = None for_graph = 0 output_section_scores = 0}
	Score = 0
...
script calc_difficulty_from_song_section \{array = None section_begin = 0 section_end = 0 index_begin = 0 fretbar_array = None fretbar_index = 0}
	Score = 0
...
script get_note_placement_value \{note1 = 0 note2 = 0 make_orange_4 = 0 only_different_notes = 0}
	note_placement = 0
	array_index = 0
	array_index2 = 0
	begin
...
script is_any_note_the_same \{note1 = 0 note2 = 0}
	if (<note1> && <note2>)
		return \{TRUE}
	endif
	return \{FALSE}
...
script da_check_for_errors \{note = 0}
	...
	if (<is_chord>)
		...
		if (<note> && ($note_bits [0]) && <note> && ($note_bits [3]) && <note> && ($note_bits [4]))
			printf \{channel = da_errors "WARNING: Green, Blue, Orange chord found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
			elseif (<note> && ($note_bits [0]) && <note> && ($note_bits [1]) && <note> && ($note_bits [4]))
			printf \{channel = da_errors "WARNING: Green, Red, Orange chord found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
			elseif (<note> && ($note_bits [0]) && <note> && ($note_bits [2]) && <note> && ($note_bits [4]))
			printf \{channel = da_errors "WARNING: Green, Yellow, Orange chord found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
			elseif (<note> && ($note_bits [0]) && <note> && ($note_bits [4]))
			printf \{channel = da_errors "WARNING: Green, Orange chord found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
		endif
		if ($current_da_diff = easy || $current_da_diff = medium)
			if (<note> && ($note_bits [0]) && <note> && ($note_bits [3]))
				printf \{channel = da_errors "WARNING: Green, Blue chord found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
			endif
			if (<num_notes> >= 3)
				printf channel = da_errors "WARNING: %c note chord found in song %a on difficulty %b" c = <num_notes> a = $current_song_qpak b = $current_da_diff
			endif
			if ($current_da_diff = easy)
				if (<note> && ($note_bits [0]) && <note> && ($note_bits [2]))
					printf \{channel = da_errors "WARNING: Green, Yellow chord found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
				endif
			endif
		endif
	else
		if ($current_da_diff = easy || $current_da_diff = medium)
			if (<note> && ($note_bits [4]))
				printf \{channel = da_errors "WARNING: Orange note found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
			endif
		endif
		if ($current_da_diff = easy)
			if (<note> && ($note_bits [3]))
				printf \{channel = da_errors "WARNING: Blue note found in song %a on difficulty %b" a = $current_song_qpak b = $current_da_diff}
			endif
		endif
	endif
endscript
