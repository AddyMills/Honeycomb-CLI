...
script add_basic_stats_desc 
	...
	if NOT ($game_mode = p2_battle || $current_song = bossjoe)
		...
		if NOT ($game_mode = p2_coop || $game_mode = p2_quickplay)
			...
			add_text_to_column \{column = 'center' text = ""}
			add_text_to_column \{column = 'center' text = "AVG MULTIPLIER" dims = (400.0, 0.0)}
		endif
	else
...
script detailed_stats_create_container \{for_practice = 0}
	...
	CreateScreenElement \{type = ContainerElement parent = root_window id = ds_container Pos = (0.0, 0.0)}
	CreateScreenElement {
		type = TextElement
		parent = root_window
		id = continue_text
