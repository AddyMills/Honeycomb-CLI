...
script create_select_practice_mode_menu 
	...
	displayText \{parent = spm_container text = "Select training mode" font = text_a11 Pos = (655.0, 540.0) rgba = [255 195 20 255] rot = -5 z = 10 noshadow just = [center center]}
...
script practice_mode_footnote \{alpha = 0}
	if NOT ScreenElementExists \{id = footnote_overlay}
		CreateScreenElement \{type = ContainerElement parent = spm_container id = footnote_overlay Pos = (0.0, 0.0) just = [left top] alpha = 0}
		CreateScreenElement \{type = ContainerElement parent = footnote_overlay id = practice_mode_footer Pos = (640.0, 645.0) z_priority = 1000 just = [center bottom] alpha = 1}
		CreateScreenElement \{type = ContainerElement parent = practice_mode_footer id = footer_gradient Pos = (0.0, -17.0) z_priority = 1000 just = [center bottom] alpha = 1 rot_angle = 90}
	...
	if (<alpha> = 1)
		...
		DoScreenElementMorph \{id = footnote_overlay alpha = 1 time = 0.25}
		RunScriptOnScreenElement \{id = practice_mode_footer_text alert_footnote}
	else
		DoScreenElementMorph \{id = footnote_overlay alpha = 0 time = 0.25}
	endif
endscript

script alert_footnote 
	begin
	DoMorph \{alpha = 0.5 time = 0.05}
	wait \{0.05 second}
	DoMorph \{alpha = 1 time = 0.05}
	wait \{0.05 second}
	repeat 3
endscript
