...
script create_cheats_menu 
	...
	if ($entering_cheat = 0)
		...
		displayText \{parent = cheats_container Pos = (910.0, 402.0) just = [RIGHT center] id = cheats_menu_title text = "CHEATS" Scale = 1.1 rgba = [240 235 240 255] font = text_a5 noshadow}
	...
	show_cheat_warning
	displaySprite \{parent = cheats_container id = cheats_hilite tex = white rgba = [150 0 50 255] rot_angle = 1 Pos = (349.0, 382.0) dims = (230.0, 20.0) z = 2}
	new_menu \{scrollid = cheats_scroll vmenuid = cheats_vmenu menu_pos = (360.0, 260.0) text_left spacing = -12 rot_angle = 1}
	LaunchEvent \{type = unfocus target = cheats_vmenu}
	...
	change \{guitar_hero_cheats_completed = [0 0 0 0 0 0 0 0]}
	LaunchEvent \{type = focus target = cheats_vmenu}
endscript
...
script cheats_zoomin_guitar 
	...
	LaunchEvent \{type = unfocus target = vmenu_new_cheat}
	Menu_Music_Off
	...
	if ScreenElementExists \{id = vmenu_new_cheat}
		LaunchEvent \{type = focus target = vmenu_new_cheat}
	endif
endscript

script cheats_do_guitar_morphs 
	if ScreenElementExists \{id = guitar_sprite}
		guitar_sprite :DoMorph \{Scale = (1.0, 1.0) Pos = (140.0, 545.0) time = 0.75 motion = ease_in}
		guitar_sprite :DoMorph \{Pos = (150.0, 545.0) time = 0.2 motion = ease_out}
		guitar_sprite :DoMorph \{Pos = (140.0, 545.0) time = 0.2 motion = ease_in}
	endif
	return
endscript
...
script cheats_zoomout_guitar 
	...
	if ScreenElementExists \{id = guitar_sprite}
		guitar_sprite :DoMorph \{Pos = (150.0, 545.0) time = 0.2 motion = ease_out}
		guitar_sprite :DoMorph \{Pos = (140.0, 545.0) time = 0.2 motion = ease_in}
		guitar_sprite :DoMorph \{Scale = (1.0, 1.0) Pos = (1400.0, 545.0) time = 0.5 motion = ease_in}
	endif
...
script cheat_award_text \{show_unlock = 1}
	...
	if ScreenElementExists \{id = cheats_menu_title}
		cheats_menu_title :DoMorph \{alpha = 0 time = 0.2}
	endif
	do_extra_exploding_particles \{z_priority = 80 Pos = (300.0, 150.0) parent = awardtext}
	begin
	awardtext :DoMorph \{alpha = 1.0 time = 0.2 motion = ease_out}
	awardtext :DoMorph \{alpha = 0.3 time = 0}
	repeat 16
	DoScreenElementMorph \{id = awardtext alpha = 0 time = 1}
	if ScreenElementExists \{id = cheats_menu_title}
		cheats_menu_title :DoMorph \{alpha = 1 time = 0.5}
	endif
endscript
