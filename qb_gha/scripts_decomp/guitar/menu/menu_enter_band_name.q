...
script create_enter_band_name_menu 
	...
	CreateScreenElement \{type = SpriteElement parent = ebn_container id = ebn_menu_backdrop texture = Venue_BG rgba = [255 255 255 255] Pos = (640.0, 360.0) Scale = (1.25, 1.4) relative_scale just = [center center] z_priority = 0}
	...
	CreateScreenElement \{type = SpriteElement parent = ebn_container id = ticket_image texture = band_name_ticket rgba = [255 255 255 255] Pos = (640.0, 360.0) Scale = (1.25, 1.4) relative_scale just = [center center] z_priority = 1}
	CreateScreenElement {
		type = SpriteElement
...
script character_blinker 
	begin
	DoMorph \{alpha = 0.2 motion = ease_in time = 0.05}
	wait \{0.05 seconds}
	DoMorph \{alpha = 0.5 motion = ease_out time = 0.05}
	wait \{0.05 seconds}
	DoMorph \{alpha = 0.2 motion = ease_in time = 0.05}
	wait \{0.05 seconds}
	DoMorph \{alpha = 0.6 motion = ease_out time = 0.2}
	wait \{0.2 seconds}
	repeat
endscript
	...
	if (($new_band_index + 1) < $max_band_characters)
		...
		if ($new_band_index > 8)
			...
			if ($enter_band_name_small_vals.text_scale < 1.2)
				change \{structurename = enter_band_name_small_vals text_scale = 1.2}
			endif
		endif
		menu_ebn_refresh_band_name
		if (($new_band_index + 1) = $max_band_characters)
...
script menu_ebn_refresh_band_name 
	...
	if (($new_band_index = 9) && ($ebn_using_big_vals = 1))
		...
		change \{structurename = enter_band_name_small_vals text_scale = 1.8}
	...
	if (<do_scaling>)
		<time> = 0.5
		LaunchEvent \{type = unfocus target = ebn_marker}
		DoScreenElementMorph id = band_name_text Scale = (<vals_struct>.text_scale) Pos = (<vals_struct>.text_pos) time = <time> motion = ease_out
		DoScreenElementMorph id = band_name_entry_char Scale = (<vals_struct>.text_scale) time = <time> motion = ease_out
		SetScreenElementProps \{id = band_name_entry_char hide}
		SetScreenElementProps \{id = ebn_marker hide}
