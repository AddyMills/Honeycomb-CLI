...
script create_band \{starting_song = FALSE async = 0}
	if ($disable_band = 1)
		return
	endif
	...
	if ($current_num_players = 1)
		...
		if StructureContains structure = ($<band>) name = vocalist2
			...
			printf \{channel = singer "Attempting to create a second vocalist"}
				...
				if (<found> = 1)
					...
					if (<char_name> = 'JoePShakinMyCage')
						...
						if (<char_name> = 'JoeP' || <char_name> = 'JoePRagdoll' || <char_name> = 'JoePsweetemotion')
							printf \{channel = animinfo "override outfit/style for joep"}
							outfit = 1
							GetPakManCurrent \{map = zones}
							switch <pak>
								case z_nipmuc
...
script create_guitarist \{name = guitarist profile_name = 'judy' instrument_id = Instrument_Les_Paul_Black async = 0 animpak = 1 starting_song = FALSE}
	...
	if (<found> = 1)
		...
		if StructureContains structure = <profile_struct> name = accessory_bones
			printf \{channel = animinfo "guitarist: accessory bone struct found in profile!"}
			accessory_bones = (<profile_struct>.accessory_bones)
			<name> :ragdoll_setaccessorybones accessory_bones = $<accessory_bones>
		else
			printf \{channel = animinfo "guitarist: accessory bone struct NOT found in profile...using default!"}
...
script create_bassist \{name = bassist profile_name = 'bassist' alt_instrument = None async = 0}
	extendcrc <name> '_Info' out = info_struct
	...
	if (<found> = 1)
		...
		if ((<profile_struct>.musician_body.desc_id) = Aero_Bassist)
			...
			switch <pak>
				case z_nipmuc
				printf \{channel = animinfo "using Guitarist_TomH_Outfit1_Style2"}
				desired_body = Guitarist_TomH_Outfit1_Style2
				case z_maxskc
				printf \{channel = animinfo "using Guitarist_TomH_Outfit1_Style3"}
				desired_body = Guitarist_TomH_Outfit1_Style3
				case z_Fenway
				printf \{channel = animinfo "using Guitarist_TomH_Outfit1_Style4"}
				desired_body = Guitarist_TomH_Outfit1_Style4
				case z_nine_lives
				printf \{channel = animinfo "using Guitarist_TomH_Outfit1_Style5"}
				desired_body = Guitarist_TomH_Outfit1_Style5
				case z_jpplay
				printf \{channel = animinfo "using Guitarist_TomH_Outfit1_Style6"}
				desired_body = Guitarist_TomH_Outfit1_Style6
				default
				printf \{channel = animinfo "using Aero_bassist"}
			endswitch
			...
			printf \{channel = animinfo "not Aero_bassist"}
		endif
		if ((<profile_struct>.musician_body.desc_id) = Aero_Guitar)
			GetPakManCurrent \{map = zones}
			switch <pak>
				case z_nipmuc
				printf \{channel = animinfo "using Guitarist_BradW_Outfit1_Style2"}
				desired_body = Guitarist_BradW_Outfit1_Style2
				case z_maxskc
				printf \{channel = animinfo "using Guitarist_BradW_Outfit1_Style3"}
				desired_body = Guitarist_BradW_Outfit1_Style3
				case z_Fenway
				printf \{channel = animinfo "using Guitarist_BradW_Outfit1_Style4"}
				desired_body = Guitarist_BradW_Outfit1_Style4
				case z_nine_lives
				printf \{channel = animinfo "using Guitarist_BradW_Outfit1_Style5"}
				desired_body = Guitarist_BradW_Outfit1_Style5
				case z_jpplay
				printf \{channel = animinfo "using Guitarist_BradW_Outfit1_Style6"}
				desired_body = Guitarist_BradW_Outfit1_Style6
				default
				printf \{channel = animinfo "using Aero_Guitar"}
			endswitch
		else
			printf \{channel = animinfo "not Aero_Guitar"}
		endif
		if (<alt_instrument> = None)
		...
		if StructureContains structure = <profile_struct> name = accessory_bones
			printf \{channel = animinfo "bassist: accessory bone struct found in profile!"}
			accessory_bones = (<profile_struct>.accessory_bones)
			<name> :ragdoll_setaccessorybones accessory_bones = $<accessory_bones>
		else
			printf \{channel = animinfo "bassist: accessory bone struct NOT found in profile...using default!"}
			<name> :ragdoll_setaccessorybones accessory_bones = $Guitarist_accessory_bones
		endif
		<name> :Obj_SwitchScript guitarist_idle
		<name> :obj_spawnscriptnow facial_anim_loop
...
script create_vocalist \{name = vocalist profile_name = 'singer' async = 0}
	...
	if (<found> = 1)
		...
		if ((<profile_struct>.musician_body.desc_id) = Aero_Singer)
			...
			switch <pak>
				case z_nipmuc
				printf \{channel = animinfo "using Aero_Singer_Nipmuc"}
				alt_body = Aero_Singer_Nipmuc
				case z_maxskc
				printf \{channel = animinfo "using Aero_Singer_MaxKC"}
				alt_body = Aero_Singer_MaxKC
				case z_Fenway
				printf \{channel = animinfo "using Aero_Singer_Orpheum"}
				alt_body = Aero_Singer_Orpheum
				case z_nine_lives
				printf \{channel = animinfo "using Aero_Singer_Nine_Lives"}
				alt_body = Aero_Singer_Nine_Lives
				case z_jpplay
				printf \{channel = animinfo "using Aero_drummer"}
				alt_body = Aero_Singer_JPPlay
				default
				printf \{channel = animinfo "using Aero_singer"}
			endswitch
		else
			printf \{channel = animinfo "not Aero_singer"}
		endif
		if (<alt_body> != None)
			profile = {<profile_struct>
		...
		if StructureContains structure = <profile_struct> name = accessory_bones
			printf \{channel = animinfo "vocalist: accessory bone struct found in profile!"}
			accessory_bones = (<profile_struct>.accessory_bones)
			<name> :ragdoll_setaccessorybones accessory_bones = $<accessory_bones>
		else
			printf \{channel = animinfo "vocalist: accessory bone struct NOT found in profile...using default!"}
			<name> :ragdoll_setaccessorybones accessory_bones = $Guitarist_accessory_bones
		endif
...
script create_drummer \{name = drummer profile_name = 'drummer' async = 0}
	...
	if (<found> = 1)
		...
		if ((<profile_struct>.musician_body.desc_id) = Aero_Drummer)
			...
			switch <pak>
				case z_nipmuc
				printf \{channel = animinfo "using Aero_Drummer_Nipmuc"}
				alt_body = Aero_Drummer_Nipmuc
				case z_maxskc
				printf \{channel = animinfo "using Aero_Drummer_MaxKC"}
				alt_body = Aero_Drummer_MaxKC
				case z_Fenway
				printf \{channel = animinfo "using Aero_Drummer_Orpheum"}
				alt_body = Aero_Drummer_Orpheum
				case z_nine_lives
				printf \{channel = animinfo "using Aero_Drummer_Nine_Lives"}
				alt_body = Aero_Drummer_Nine_Lives
				case z_jpplay
				printf \{channel = animinfo "using Aero_Drummer_JPPlay"}
				alt_body = Aero_Drummer_JPPlay
				default
				printf \{channel = animinfo "using Aero_drummer"}
			endswitch
		else
			printf \{channel = animinfo "not Aero_drummer"}
		endif
		if (<alt_body> != None)
			profile = {<profile_struct>
				musician_body = {desc_id = <alt_body>}
		...
		if StructureContains structure = <profile_struct> name = accessory_bones
			printf \{channel = animinfo "drummer: accessory bone struct found in profile!"}
			accessory_bones = (<profile_struct>.accessory_bones)
			<name> :ragdoll_setaccessorybones accessory_bones = $<accessory_bones>
		else
			printf \{channel = animinfo "drummer: accessory bone struct NOT found in profile...using default!"}
			<name> :ragdoll_setaccessorybones accessory_bones = $Guitarist_accessory_bones
		endif
...
script play_special_anim \{stance = stance_a disable_arms = 2 blendduration = 0.2}
	...
	if hero_play_tempo_anim_cfunc anim = <anim> blendduration = <blendduration>
		if (<anim_to_run> = GH3_Guit_JoeP_Whip_Win)
			...
			ragdoll_setaccessorybones \{accessory_bones = $Guitarist_accessory_bones disable}
		endif
		hero_play_anim anim = <anim_to_run> blendduration = <blend_duration> usemotionextraction = <use_motion_extraction>
	endif
...
script play_intro_anims 
	...
	Band_ChangeStance \{name = rhythm stance = stance_a}
	Band_ChangeStance \{name = vocalist stance = stance_a}
	Band_ChangeStance \{name = vocalist2 stance = stance_a}
...
script play_win_anims 
	...
	if CompositeObjectExists \{name = rhythm}
		rhythm :Obj_SwitchScript \{play_special_anim params = {stance = win anim = idle blendduration = 0.0}}
	endif
	if CompositeObjectExists \{name = vocalist}
		vocalist :Obj_SwitchScript \{play_special_anim params = {stance = win anim = idle blendduration = 0.0}}
	endif
	if CompositeObjectExists \{name = vocalist2}
		vocalist2 :Obj_SwitchScript \{play_special_anim params = {stance = win anim = idle blendduration = 0.0}}
	endif
	restore_idle_faces
endscript
...
script play_lose_anims 
	...
	if CompositeObjectExists \{name = rhythm}
		rhythm :Obj_SwitchScript \{play_special_anim params = {stance = lose anim = idle blendduration = 0.0}}
	endif
	if CompositeObjectExists \{name = drummer}
		change \{structurename = drummer_info last_left_arm_note = 0}
		change \{structurename = drummer_info last_right_arm_note = 0}
	...
	if CompositeObjectExists \{name = vocalist2}
		vocalist2 :Obj_SwitchScript \{play_special_anim params = {stance = lose anim = idle blendduration = 0.0}}
	endif
	restore_idle_faces
endscript
...
script create_vocalist_dummy 
	if NOT CompositeObjectExists \{vocalist}
		CreateCompositeObjectInstance \{priority = $coim_priority_permanent heap = generic Components = [{Component = motion}] params = {name = vocalist_head_dummy}}
		...
		CreateCompositeObjectInstance \{priority = $coim_priority_permanent heap = generic Components = [{Component = motion}] params = {name = vocalist_dummy}}
		get_start_node_id \{member = vocalist}
		GetWaypointPos name = <waypoint_id>
		GetWaypointDir name = <waypoint_id>
