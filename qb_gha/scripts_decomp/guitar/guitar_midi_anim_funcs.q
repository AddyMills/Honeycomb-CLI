...
script Band_PlaysimpleAnim \{name = guitarist allow_in_2player = FALSE}
...
script Band_ChangeStance \{name = guitarist stance = stance_a allow_in_2player = TRUE}
	if NOT CompositeObjectExists name = <name>
		return
	endif
	if ($current_num_players = 2)
...
script Band_WalkToNode \{name = guitarist faceAudience = TRUE allow_in_2player = FALSE}
	if NOT CompositeObjectExists name = <name>
		return
	endif
	if ($current_num_players = 2)
