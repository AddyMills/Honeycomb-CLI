...
script start_gem_scroller \{starttime = 0 practice_intro = 0 training_mode = 0 endtime = 99999999 devil_finish_restart = 0 end_credits_restart = 0}
	...
	if (<training_mode> = 0)
		if NOT create_band \{starting_song = TRUE async = 1}
			downloadcontentlost
		endif
	endif
	...
	if ($is_network_game)
		...
		if (($net_ready_to_start) || ($player2_present = 0))
			...
			ui_print_gamertags \{pos1 = (240.0, 70.0) pos2 = (1040.0, 70.0) dims = (310.0, 25.0) just1 = [left top] just2 = [RIGHT top] offscreen = 1}
			break
		endif
		wait \{1 gameframe}
		repeat
...
script kill_gem_scroller \{no_render = 0 reset_character_info = 1}
	mark_unsafe_for_shutdown
	...
	if (<reset_character_info> = 1)
		printf \{channel = reset "resetting character info......."}
		...
		printf \{channel = reset "NOT resetting character info......."}
	endif
	if ($debug_showmeasures = on)
